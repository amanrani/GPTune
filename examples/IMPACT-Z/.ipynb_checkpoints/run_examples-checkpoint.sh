...
if [[ -z "${GPTUNE_LITE_MODE}" ]]; then
  ...
  $MPIRUN --oversubscribe --allow-run-as-root --mca btl self,tcp,vader --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1  python ./impact-z_single.py -ntask 1 -nrun 100 -optimization $tuner | tee a.out_${tuner}
else
